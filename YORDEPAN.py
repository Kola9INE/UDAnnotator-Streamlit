...
DEPREL = {'acl':'clausal modifier of noun (adnominal clause)',
                    ...
                    'comp:aux':'auxilliary complement',
                    'conj':'conjunct',
                    'cop':'copula',
...
def intro():
    ...
    with st.expander('`🤔💭 WHAT IS THIS ABOUT?`'):
        st.markdown('''\n
                The dependency parsing is one of the many approaches to answering the question: `HOW CAN A MACHINE UNDERSTAND THE SYNTAX OF A LANGUAGE?` This approach seeks to present a language's syntax as a `dependency graph` with individual tokens as `nodes` and their syntactic relationship among one another as `edges` (Mark-Jan Nederhof & Giorgio Satta, 2010:120).
                With a language's syntax now represented as a graph of dependencies, a machine can understand how each words relates to one another in a sentence and can achieve tasks like translation from one language to another, or even communicate to us like a human!
        ...
        if edit_table:
            ...
            if cnlu:
                ...
                if len(ID) != len(tokens):
                    ...
                    for i in st.session_state.DATA.itertuples():
                        new.append(f'{i.ID}\t{i.FORM}\t{i.LEMMA}\t{i.UPOS}\t{i.XPOS}\t{i.FEATS}\t{i.HEAD}\t{i.DEPREL}\t{i.DEPS}\t{i.MISC}')
                    new = '\n'.join(new)
